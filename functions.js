...
/********************************************************************
2016.6.14
十五：获取元素到浏览器的距离:offset(obj);
left=所有具有定位属性的父元素的offsetLeft+所有具有定位属性的父元素的border-left+自身的offsetLeft;
1，获取所有的具有定位属性的父元素；判断:position=relative/absolute;
2，计算所有的offsetLeft + borderLeftWidth;
3，返回最终的结果{left:200px;top:300px}
4，调用方式offset(one).left;
**/
function offset(obj){
	//获取具有定位属性的父元素部分
...
2016.6.15
十六：滚轮事件的兼容函数；
调用方式：mouseWheel(obj,function(){},function(){});
